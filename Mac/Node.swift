...
private extension State {
    var prospect: Bool { return false }
...
class Node: NSView, State {
    ...
    fileprivate weak var close: Button?
    var prospect: Bool { return state.prospect }
...
private class Code: State {
    fileprivate var prospect: Bool { return true }
    ...
    fileprivate func update(_ node: Node!) {
        ...
        let close = Button("remove", target: node, action: #selector(node.remove))
